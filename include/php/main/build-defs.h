...
#define CONFIGURE_COMMAND " './configure'  '--prefix=/usr/local/php' '--enable-fpm' '--with-fpm-user=www-data' '--with-fpm-group=www-data' '--with-config-file-path=/usr/local/php/lib' '--with-mysql' '--with-mysqli' '--with-pdo_mysql' '--with-openssl' '--enable-zip'"
